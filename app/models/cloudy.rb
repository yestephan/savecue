class Cloudy
  def self.perform
    ...
    # check all locations with cloudy and create an array with their names
    cloudy_locations = []
    locations[:coordinates].each do |location,coordinates|
      did_it_cloudy = weather_lookup(coordinates)
      if did_it_cloudy
        cloudy_locations.append(location)
      end
      p "Dit it cloudy in #{location}? #{did_it_cloudy}"
    end

    # iterate through the cloudy locations and create a transaction for each user_cue related to them
    cloudy_locations.each do |location|
  ...
  def self.query_all_locations
    # Get all cloudy cues from user_cues
    all_cues = Cue.find_by({category: "cloudy"}).user_cues
  ...
  def self.weather_lookup(coordinates)
    ...
    result["hourly"].each do |hour|
      # in case any kind of cloudy is detected set the variable to true
      weather = hour["weather"].select { |item| item["main"].downcase == "clouds" }
      is_it_cloudy |= weather.count > 0
    end
